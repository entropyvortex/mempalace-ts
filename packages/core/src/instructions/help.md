...
## CLI Commands
    ...
    mempalace-ts init <dir>                  Initialize a new palace
    mempalace-ts mine <dir>                  Mine a project (default mode)
    mempalace-ts mine <dir> --mode convos    Mine conversation exports
    mempalace-ts search "query"              Search your memories
    mempalace-ts split <dir>                 Split large transcript files
    mempalace-ts wake-up                     Load palace into context
    mempalace-ts compress                    Compress palace storage
    mempalace-ts status                      Show palace status
    mempalace-ts repair                      Rebuild vector index
    mempalace-ts hook run                    Run hook logic (for harness integration)
    mempalace-ts instructions <name>         Output skill instructions
...
Hooks read JSON from stdin and output JSON to stdout. They can be invoked via:
    ...
    echo '{"session_id":"abc","stop_hook_active":false,"transcript_path":"..."}' | mempalace-ts hook run --hook stop --harness claude-code
