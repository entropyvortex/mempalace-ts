...
- If MCP is not available: Run the CLI command: mempalace-ts status
...
- Empty palace (zero memories): Suggest "Try mempalace-ts mine <dir> to add
  data from files."
- Has data but no knowledge graph (memories exist but KG stats show zero
  ...
  queries."
- Healthy palace (has memories and KG data): Suggest "Use mempalace-ts search
  to query your memories."
