...
- **`MEMPAL_DIR`** — Optional. Set to a conversations directory to auto-run `mempalace-ts mine <dir>` on each save trigger. Leave blank (default) to let the AI handle saving via the block reason message.
...
```bash
mempalace-ts mine <dir>               # Mine all files in a directory
mempalace-ts mine <dir> --mode convos # Mine conversation transcripts only
```
